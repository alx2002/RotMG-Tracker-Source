...
export class Client {
  ...
  constructor(server: Server, buildVersion: string, accInfo: Account) {
    ...
    console.log(this.alias, `Starting connection to ${server.name}`, LogLevel.Info);
    this.connect();
  }
  ...
  setProxy(proxy: Proxy): void {
    if (proxy) {
      console.log(this.alias, 'Connecting to new proxy.');
    } else {
      console.log(this.alias, 'Connecting without proxy.');
    }
    this.proxy = proxy;
    this.connect();
  }
  ...
  connectToServer(server: Server): void {
    console.log(this.alias, `Switching to ${server.name}.`, LogLevel.Info);
  ...
  changeGameId(gameId: GameId): void {
    console.log(this.alias, `Changing gameId to ${gameId}`, LogLevel.Info);
    this.gameId = gameId;
    this.connect();
  }
  ...
  findPath(to: Point): void {
    if (!this.pathfinderEnabled) {
      console.log(this.alias, 'Pathfinding is not enabled. Please enable it in the acc-config.', LogLevel.Warning);
      return;
    }
    ...
    this.pathfinder.findPath(this.worldPos.toPoint(), to).then((path) => {
      ...
      console.log(this.alias, `Error finding path: ${error.message}`, LogLevel.Error);
    });
  }
  ...
  private damage(damage: number, bulletId: number, objectId: number): void {
    ...
    console.log(this.alias, `Took ${actualDamge} damage. At ${Math.round(this.playerData.hp)} health.`);
    if (this.playerData.hp <= this.playerData.maxHP * 0.3) {
      console.log(this.alias, `Auto nexused at ${Math.round(this.playerData.hp)} HP`, LogLevel.Warning);
      this.clientSocket.destroy();
      return;
    }
          ...
          for (let i = 0; i < this.projectiles.length; i++) {
            ...
            if (this.projectiles[i].damagePlayers) {
              ...
              if (closestEnemy) {
                ...
                if (lastUpdate > 400) {
                  console.log(this.alias, `Preventing EnemyHit. Time since last update: ${lastUpdate}`, LogLevel.Debug);
                  this.projectiles.splice(i, 1);
                  continue;
                }
  ...
  private onMapInfo(client: Client, mapInfoPacket: MapInfoPacket): void {
    if (this.charInfo.charId > 0) {
      ...
      console.log(this.alias, `Connecting to ${mapInfoPacket.name}`, LogLevel.Info);
      ...
      console.log(this.alias, 'Creating new char', LogLevel.Info);
  ...
  private onFailurePacket(client: Client, failurePacket: FailurePacket): void {
    switch (failurePacket.errorId) {
      case FailureCode.IncorrectVersion:
        console.log(this.alias, 'buildVersion out of date. Updating and reconnecting...');
      ...
      case FailureCode.InvalidTeleportTarget:
        console.log(this.alias, 'Invalid teleport target.', LogLevel.Warning);
        break;
      case FailureCode.EmailVerificationNeeded:
        console.log(this.alias, 'Email verification is required for this account.', LogLevel.Error);
        break;
      case FailureCode.BadKey:
        console.log(this.alias, 'Invalid key used.', LogLevel.Error);
        this.key = [];
        this.gameId = -2;
        this.keyTime = -1;
        break;
      default:
        console.log(this.alias, `Received failure ${failurePacket.errorId}: "${failurePacket.errorDescription}"`, LogLevel.Error);
        break;
    }
  }
  ...
  private onCreateSuccess(client: Client, createSuccessPacket: CreateSuccessPacket): void {
    console.log(this.alias, 'Connected!', LogLevel.Success);
  ...
  private onConnect(): void {
    console.log(this.alias, `Connected to ${this.internalServer.name}!`, LogLevel.Success);
  ...
  private onClose(error: boolean): void {
    console.log(this.alias, `The connection to ${this.internalServer.name} was closed.`, LogLevel.Warning);
    this.socketConnected = false;
    Client.emitter.emit('disconnect', this);
    this.nextPos.length = 0;
    this.pathfinderTarget = null;
    ...
    console.log(this.alias, `Reconnecting in ${reconnectTime} seconds`);
  ...
  private onError(error: Error): void {
    console.log(this.alias, `Received socket error: ${error.message}`, LogLevel.Error);
  }
  ...
  private connect(): void {
    ...
    if (this.proxy) {
      console.log(this.alias, 'Establishing proxy', LogLevel.Info);
      SocksClient.createConnection({
        ...
        console.log(this.alias, 'Established proxy!', LogLevel.Success);
        this.clientSocket = info.socket;
        this.initSocket(false);
      }).catch((error) => {
        console.log(this.alias, 'Error establishing proxy', LogLevel.Error);
        console.log(this.alias, error, LogLevel.Error);
      });
    } else {
  ...
  private initSocket(connect: boolean): void {
    ...
    this.packetio.on('error', (err: Error) => {
      console.log(this.alias, `Received PacketIO error: ${err.message}`, LogLevel.Error);
      this.clientSocket.destroy();
    });
    if (connect) {
